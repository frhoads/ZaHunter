...
-(void)searchForPizza
{
    ...
    searchRequest.region = MKCoordinateRegionMakeWithDistance(currentLocation.coordinate, 3000, 3000);
    ...
    [search startWithCompletionHandler:^(MKLocalSearchResponse *response, NSError *error) {
        ...
        for (NSArray *slice in pizzaPlaces) {
...
-(void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray *)locations
{
    for (CLLocation *phoneLocation in locations) {
        if (phoneLocation.verticalAccuracy > 100 || phoneLocation.horizontalAccuracy > 100)
            continue;
